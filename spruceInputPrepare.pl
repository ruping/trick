...
while ( <IN> ){
  ...
  if (/^sample\_index/){   #header
    ...
    my ($sample_index, $sample_label, $character_label, $character_index, $vaf_lb, $vaf_mean, $vaf_ub, $x1, $y1, $mu1, $seg) = split /\t/;
    my $buffer = join("\t",$sample_index, $sample_label, $character_label, $character_index, $vaf_lb, $vaf_mean, $vaf_ub, $x1, $y1, $mu1);
    $buffer =~ s/\tNA//g;
    $print .= $buffer."\n";
